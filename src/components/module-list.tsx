...
import { CheckCircle2, Circle, LockKeyhole, Play } from "lucide-react";
  ...
  return (
    <div className="grid gap-3">
      {level.modules.map((module, index) => {
        ...
        const previousModule = level.modules[index - 1];
        const isLocked = Boolean(previousModule && (!progress.ready || !progress.isLessonComplete(`${level.id}:${previousModule.id}`)));
        const content = (
          <>
            <div className="flex items-center gap-3 sm:block">
              <span className={`grid size-11 place-items-center rounded-lg text-sm font-black text-white ${isLocked ? "bg-[var(--muted)]" : "bg-[var(--ink)]"}`}>
                {module.number}
              </span>
              <span className="sm:sr-only">{isLocked ? "Locked" : isDone ? "Complete" : "Open"}</span>
            </div>
            <div>
              <h2 className="text-lg font-black tracking-normal">{module.title}</h2>
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">
                {isLocked ? `Complete ${previousModule?.number} first to unlock this module.` : module.coreConcept}
              </p>
            </div>
            <div className="flex items-center gap-3 text-sm font-black">
              {isLocked ? (
                <span className="inline-flex items-center gap-1 rounded-md bg-[rgba(30,37,36,0.08)] px-2 py-1 text-[var(--muted)]">
              ...
              {isDone ? (
                ...
                <LockKeyhole className="text-[var(--muted)]" size={20} aria-hidden="true" />
              ) : (
        ...
        return isLocked ? (
          <div
            key={module.id}
            aria-disabled="true"
            className="grid gap-4 rounded-lg border border-[var(--line)] bg-[var(--panel)] p-4 opacity-70 sm:grid-cols-[96px_1fr_auto] sm:items-center"
          >
            {content}
          </div>
        ) : (
